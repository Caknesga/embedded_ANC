...
void app_main(void)
{
    ...
    // USB CDC
    esp_vfs_dev_cdcacm_register(NULL);

    uint8_t buf[64];

    while (1) {
        int n = read(0, buf, sizeof(buf));
        if (n > 0) uart_write_bytes(UART_NUM, buf, n);

        n = uart_read_bytes(UART_NUM, buf, sizeof(buf), 1);
        if (n > 0) write(1, buf, n);
    }
}


//test for microphone recording to RAM and dumping via serial
/*#include <stdio.h>
